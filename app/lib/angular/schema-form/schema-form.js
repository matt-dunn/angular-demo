            ...
            service.merge = function(schema, form, ignore, options, readonly) {
                ...
                return postProcessFn(form.map(function(obj) {
                    ...
                    if (obj.key) {
                        ...
                        if (lookup[strid]) {
                            if (options.formDefaults && options.formDefaults.validationMessage) {
                                obj.validationMessage = angular.extend({}, options.formDefaults.validationMessage, obj.validationMessage);
                            }
                        ...
                        if (form.items.length > 1) {
                            subForm = {
                                ...
                                items: form.items.map(function(item) {
                                    ...
                                    if (form.readonly === true) { // Inheriting false is not cool.
...
angular.module('schemaForm').directive('schemaValidate', ['sfValidator', '$q', function(sfValidator, $q) {
    return {
        restrict: 'A',
        scope: false,
        ...
        link: function(scope, element, attrs, ngModel) {
            ...
            // @PATCH: added setErrorCode to allow error codes to be set by custom validators
            scope.setErrorCode = function(code) {
                error = {
                    code: code
                };
            };
